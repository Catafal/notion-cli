...
- **Markdown parser enhancements** — checkboxes (`- [ ]`/`- [x]` → `to_do` blocks), strikethrough (`~~text~~`), and markdown tables (with header detection, rich text in cells). Applies to `page create -f`, `append`, `daily`, `quick`, and `template use`.
